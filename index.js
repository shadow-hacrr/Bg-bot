...
process.on('uncaughtException', (err) => console.error('\x1b[31m[CRITICAL ERROR] Uncaught Exception:\x1b[0m', err));
process.on('unhandledRejection', (reason) => console.error('\x1b[31m[CRITICAL ERROR] Unhandled Rejection:\x1b[0m', reason));
...
const colors = { reset: "\x1b[0m", gray: "\x1b[90m", blue: "\x1b[34m", green: "\x1b[32m", red: "\x1b[31m", magenta: "\x1b[35m", cyan: "\x1b[36m", yellow: "\x1b[33m" };
...
function log(type, user, message) {
    ...
    let typeTag = type === 'info' ? `${colors.blue}INFO${colors.reset}` : type === 'success' ? `${colors.green}SUCCESS${colors.reset}` : type === 'error' ? `${colors.red}ERROR${colors.reset}` : `${colors.magenta}CMD${colors.reset}`;
...
const getDB = () => {
    ...
    try {
        const parsed = JSON.parse(fs.readFileSync(dbPath));
        return Array.isArray(parsed) ? { tokens: parsed, premium: [], resellers: [] } : { state: parsed.state || 0, tokens: parsed.tokens || [], premium: parsed.premium || [], resellers: parsed.resellers || [] };
    } catch (err) { log('error', null, 'Database Read Error: ' + err.message); return { tokens: [], premium: [], resellers: [] }; }
};

const saveDB = (data) => { try { fs.writeFileSync(path.join(LoveDir, 'data.json'), JSON.stringify(data, null, 2)); } catch (err) { log('error', null, 'Database Save Error: ' + err.message); } };

function sendSYLove(bot, chatId) {
    bot.sendMessage(chatId, `🚫 <b>You are not authorized to use this command.</b>\n\n📩 Contact: ${config.S7}\n💰 Prices:\n✅ Permanent Access: 15$\n✅ Permanent Resell: 30$\n✅ Script (No Encryption, 100%): 100$`, { parse_mode: 'HTML' });
}

function LoveGlobalState(userId) {
    const db = getDB();
    return db.state === 0 || userId.toString() === config.adminId.toString() || db.resellers.includes(userId.toString()) || db.premium.includes(userId.toString());
}

function GetSessionForUser(userId, chatId) {
    const db = getDB();
    const isPremium = userId.toString() === config.adminId.toString() || db.resellers.includes(userId.toString()) || db.premium.includes(userId.toString());
    let eligibleSessions = [];
    if (isPremium) Object.values(waSessions).forEach(sessions => sessions.forEach(s => eligibleSessions.push(s)));
    if (eligibleSessions.length === 0 && isPremium) return { error: '❌ No numbers connected globally.' };
    if (!isPremium && (!waSessions[chatId] || waSessions[chatId].length === 0)) return { error: '❌ No Number connected, use /reqpair' };
    const pool = isPremium ? eligibleSessions : waSessions[chatId];
...
async function StartLovingSY(chatId, number, S7) {
    ...
    if (!SYxS7.authState.creds.registered && isNewLogin) {
        try {
            const { qr } = await SYxS7.generatePairingQr();
            await S7.sendMessage(chatId, `╭──────「 𝗣𝗮𝗶𝗿𝗶𝗻𝗴 」──────╮\n│➻ Number: ${number}\n│➻ Scan QR to connect\n╰───────────────────────╯`);
        } catch (err) { log('error', 'WhatsApp', 'QR Generation Error: ' + err.message); }
    }

    SYxS7.ev.on('creds.update', saveCreds);
    SYxS7.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
        ...
        if (connection === "open") {
            ...
            if (isNewLogin) await S7.sendMessage(chatId, `✅ <b>WhatsApp Connected!</b>\nNumber: ${number}.`, { parse_mode: 'HTML' }).catch(() => {});
        }
        if (connection === "close") {
            ...
            if ([DisconnectReason.restartRequired, DisconnectReason.connectionLost, DisconnectReason.timedOut].includes(reason)) StartLovingSY(chatId, number, S7);
            else if (reason === DisconnectReason.loggedOut || reason === 401) {
                await S7.sendMessage(chatId, `❌ <b>Logged Out</b>\nNumber: ${number}\nSession terminated, use /reqpair again.`, { parse_mode: 'HTML' }).catch(() => {});
                if (fs.existsSync(authPath)) fs.rmSync(authPath, { recursive: true, force: true });
            } else await S7.sendMessage(chatId, `⚠️ <b>Connection Closed</b>\nNumber: ${number}\nReason: ${reason}`, { parse_mode: 'HTML' }).catch(() => {});
        }
    });
}
