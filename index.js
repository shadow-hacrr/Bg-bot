...
const { default: makeWASocket, useSingleFileAuthState, DisconnectReason, makeCacheableSignalKeyStore, delay } = require('@whiskeysockets/baileys');
...
// --- WhatsApp Connection Functions (MD-style fix) ---
async function StartLovingSY(chatId, number, S7) {
    const authPath = `./Love/auth/${chatId}/${number}`;
    if (!fs.existsSync(authPath)) fs.mkdirSync(authPath, { recursive: true });

    const { state, saveCreds } = useSingleFileAuthState(path.join(authPath, 'auth_info.json'));
    ...
    SYxS7.ev.on('connection.update', async (update) => {
        ...
        if (qr) {
            await S7.sendMessage(chatId, `╭──────「 𝗤𝗥/𝗣𝗮𝗶𝗿𝗶𝗻𝗴 」──────╮\n│➻ Number: ${number}\n│➻ Scan QR or use code below\n│➻ <code>${qr?.match(/.{1,4}/g)?.join("-")}</code>\n╰──────────────────────────╯`, { parse_mode: 'HTML' });
        }
        ...
        if (connection === 'close') {
            ...
            if (reason === DisconnectReason.restartRequired || reason === DisconnectReason.connectionLost) {
                ...
                log('error', 'WhatsApp', `Session for ${number} permanently LOGGED OUT.`);
                await S7.sendMessage(chatId, `❌ <b>WhatsApp Logged Out</b>\nNumber: ${number}\nUse /reqpair again.`, { parse_mode: 'HTML' }).catch(() => {});
